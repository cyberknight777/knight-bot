...
+ =/joke [type]= - Sends a random joke if the type is not provided. Available types: Misc, Programming, Dark, Pun, Spooky, Christmas.
...
+ =/probe= Probe through a provided MediaTek partition to view the contents.
+ =/rtfm= - Sends a RTFM text.
+ =/run= - Runnns :).
+ =/sauce= - Provides the link to the source code of this bot.
+ =/smsg= - Bot replies with your text and deletes original message.
